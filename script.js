...
function addRow(isInitial = false) {
    ...
    const codeVis    = document.getElementById('col-code')?.checked;
    const nameVis    = document.getElementById('col-name')?.checked;
    const sectionVis = document.getElementById('col-section')?.checked;

    row.innerHTML = `
        <td class="row-number">${rowCount}</td>
        <td class="col-code" style="display:${codeVis ? 'table-cell' : 'none'}">
            <input type="text" class="course-code" placeholder="e.g. CS1002">
        </td>
        <td class="col-name" style="display:${nameVis ? 'table-cell' : 'none'}">
            <input type="text" class="course-name" placeholder="e.g. Programming Fundamentals">
        </td>
        <td class="col-section" style="display:${sectionVis ? 'table-cell' : 'none'}">
            <input type="text" class="course-section" placeholder="e.g. BCS-1C">
        </td>
...
/**
 * Update the course count badge in the portlet header
 */
function updateCourseCount() {
    const count = tbody.querySelectorAll('tr').length;
    const badge = document.getElementById('courseCount');
    if (badge) badge.textContent = count + (count === 1 ? ' Course' : ' Courses');
}


/**
 * Show/hide optional columns (code, name, section)
 */
function toggleColumn(col, show) {
    const display = show ? 'table-cell' : 'none';
    // Header cells
    document.querySelectorAll(`#courseTable .col-${col}`).forEach(el => {
        el.style.display = display;
    });
    // Body cells — update existing rows
